...
def Mean(P,psi):
	...
	# Output: (\langle psi | P | psi \rangle)
	m = pauli_to_matrix(P)
	psi_dag = psi.conj().T
	mean = (psi_dag.dot(m).dot(psi))
	return mean
...
# def cost_Var_old(CZMZ,EMj):
# 	# return epsilon^2
# 	return np.sum(Hadamard_divide(CZMZ,EMj.dot(EMj.T)))

# def cost_old(A,aaa,error=None,shots=None,Z=None):
...
# 	V = cost_Var_old(CZMZ,EMj)
# 	s = p
# 	buckets = [0]*(r-p)
# 	if (shots != None):
# 		for s in range(p+1,shots+1):
# 			VV = [cost_Var_old(CZMZ+CZMZ_plus[c+p],EMj+EMj_plus[c+p]) for c in range(r-p)]
# 			bucket = np.argmin(VV)
# 			buckets[bucket] += 1
# 			V = VV[bucket]
# 			bucket += p
...
# 		for i in range(p):
# 			print(i,sum(buckets[k]*(i in aaa[k]) for k in range(len(aaa))))
# 		print(aaa)
# 		print(buckets)
# 		return s*V
# 	if (error != None):
# 		while V > error:
# 			VV = [cost_Var_old(CZMZ+CZMZ_plus[c],EMj+EMj_plus[c]) for c in range(r)]
# 			bucket = np.argmin(VV)
# 			V = VV[bucket]
# 			CZMZ = CZMZ+CZMZ_plus[bucket]
# 			EMj = EMj+EMj_plus[bucket]
...
def cost_old(A,aaa,error=None,shots=None,Z=None):
	# bucket filling algorithm for calculating cost
		# begin by giving 2/3 measurements to each Pauli
	# return cost as (number of measurements)*(sample variance)
	p = A.ord()
	bbb = [[b] for b in range(p)]+aaa
	r = len(bbb)
	...
	CZMZ_plus = []
	for c1 in range(r):
		I = np.eye(1, r*r, k=c1*r+c1).reshape(r,r)
		CZMZ_plus.append(Hadamard_multiply(C,Z.dot(I).dot(Zt)))
	EMj = E.dot(M).dot(j)
	EMj_plus = []
	for c1 in range(r):
	...
	V = cost_Var_old(CZMZ,EMj)
	s = 0
	CZMZ_max = Hadamard_multiply(C,Z.dot(np.identity(r)).dot(Zt))
	EMj_max = E.dot(np.identity(r)).dot(j)
	buckets = [0]*(r-p)
	if (shots != None):
		for s in range(2,shots+1):
			VV = [cost_Var_old(CZMZ+CZMZ_plus[c+p],EMj+EMj_plus[c+p]) for c in range(r-p)]
			bucket = np.argmin(VV)
			buckets[bucket] += 1
			V = VV[bucket]
			...
			EMj = EMj+EMj_plus[bucket]
		for i in range(p):
			print(i,1/p+sum(buckets[k]*(i in aaa[k]) for k in range(len(aaa))))
		return s*V
	if (error != None):
		while V > error:
			VV = [cost_Var_old(CZMZ+CZMZ_plus[c+p],EMj+EMj_plus[c+p]) for c in range(r-p)]
...
def cost_Var(C,aaa,mm):
	p = C.shape[0]
	r = len(aaa)
	M = np.array([[sum(((b1 in aaa[c])&(b2 in aaa[c]))*mm[c] for c in range(r)) for b1 in range(p)] for b2 in range(p)],dtype=float)
	for a in range(p):
		M[a,a] = max(M[a,a],max(C[a,a],8/9))
	return sum(C[a1,a2]*M[a1,a2]/(M[a1,a1]*M[a2,a2]) for a1,a2 in itertools.product(range(p),repeat=2))

def optimal_clique(C,aaa,mm):
	r = len(mm)
	clique_index = -1
	V = cost_Var(C,aaa,mm)
	for b in range(r):
		nn = [mm[c]+(c==b) for c in range(r)]
		V2 = cost_Var(C,aaa,nn)
		if V2 < V:
			V = V2
			clique_index = b
	return clique_index

def cost(A,aaa,error=None,shots=None,Z=None):
	if (shots == None) and (error == None):
		raise Exception("Must define either error or shots")
	p = A.ord()
	r = len(aaa)
	C = A.adj
	...
	V = cost_Var(C,aaa,mm)
	s = 0
	while ((shots != None) and (s < shots)) or ((shots == None) and (error != None) and (V > error)):
		mm[optimal_clique(C,aaa,mm)] += 1
		V = cost_Var(C,aaa,mm)
		s += 1
	# for i in range(p):
	# 	print(i,sum(mm[j]*(i in aaa[j]) for j in range(r)))
	return s*V


def cost_Var_new(C,aaa,mm):
	p = C.shape[0]
	r = len(aaa)
	M = np.array([[sum(((b1 in aaa[c])&(b2 in aaa[c]))*mm[c] for c in range(r)) for b1 in range(p)] for b2 in range(p)],dtype=float)
	for a in range(p):
...
def optimal_clique_new(C,aaa,mm):
	r = len(mm)
	clique_index = -1
	V = cost_Var_new(C,aaa,mm)
	for b in range(r):
		nn = [mm[c]+(c==b) for c in range(r)]
		V2 = cost_Var_new(C,aaa,nn)
		if V2 < V:
			V = V2
			clique_index = b
	return clique_index

def cost_new(A,aaa,error=None,shots=None,Z=None):
	if (shots == None) and (error == None):
		raise Exception("Must define either error or shots")
	p = A.ord()
	r = len(aaa)
	...
	s = 0
	while ((shots != None) and (s < shots)) or ((shots == None) and (error != None) and (V > error)):
		mm[optimal_clique_new(C,aaa,mm)] += 1
		V = cost_Var_new(C,aaa,mm)
		s += 1
	# for i in range(p):
	# 	print(i,sum(mm[j]*(i in aaa[j]) for j in range(r)))
